...
@php
$action = $integration
? route('settings.texnity.update', $integration->id)
: route('settings.texnity.store');
$method = $integration ? 'PATCH' : 'POST';
$config = $integration->configurations ?? [];
@endphp

<form id="texnity-form" data-save-bar action="{{ $action }}" method="POST">
    @csrf
        ...
        <s-section>
            <s-stack direction="block" gap="base">
                <s-stack direction="inline" gap="small-100" alignItems="center">
                    <s-heading>Texnity configuration</s-heading>
...
<script>
    document.getElementById('texnity-form').addEventListener('submit', async function(e) {
        e.preventDefault();
